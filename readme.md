...
2.支持 GIF,PNG,JPEG,WEBP 四种常用图片裁剪。具体因浏览器支持而异 
3.支持圆形裁剪 
4.支持任意宽高调整
5.支持固定尺寸导出和实际裁剪尺寸导出
6.丰富的自定义配置。
7.支持 IE10（含）以上浏览器
8.支持 AMD,CMD,commonJS,ES6 导入

---
###用法
const crop = new Crop(options)<br>
...
github 地址：https://github.com/gapkukb/imageCrop
demo 地址:http://ic.gogogoup.com/<br />
`注意:` dist 文件夹下的 core.js 不包含预览功能，如要要使用预览功能，请使用 preview.js
###options一览 ` 除view属性是必填外，其他都为选填`
| 参数 | 说明 | 类型 | 默认值 |
| ---- |-----|------|-------|
...
---
crop 实例方法：
loadImage(url) 如果需要支持在线图片裁剪，则直接调用该方法即可，需要注意的是可能会有跨域问题，请自行斟酌
cropped() 导出图片裁剪结果,`当options.blob为true时，导出为promise,否则导出base64,注意判断值是否存在`
reset() 重置选取框到初始状态
clear() 内存回收
